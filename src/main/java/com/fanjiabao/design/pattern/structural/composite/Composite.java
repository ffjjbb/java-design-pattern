...
/**
 * @author: FanJiaBao
 * @createDate: 2026/4/20 9:27
 * @description: 结构型模式.组合模式:
...
public class Composite {
     ...
     *  - 组合模式为树形结构的面向对象实现提供了一种灵活地解决方案, 通过叶子节点和树枝节点的递归组合, 可以形成复杂的
     *  树形结构, 但对树形结构的控制却非常简单。
     * <p>
     * 分类:
     *  透明组合模式:
     *      - 透明组合模式中, 抽象根节点角色中声明了所有用于管理成员对象的方法, 比如在示例中 "MenuComponent" 声明了 "add", "remove" ...
     *      方法, 这样做的好处是确保所有的构件类都有相同的接口。透明组合模式也是组合模式的标准形式。
     *      - 透明组合模式的缺点是不够安全, 因为叶子对象和容器对象在本质上是有区别的, 叶子对象不可能有下一个层次的对象, 即不可能包含成员对象,
     *      因此为其提供 add(), remove() 等方法是没有意义的, 这在编译阶段不会出错, 但在运行阶段如果调用这些方法可能会出错(如果没有提供相应的
     *      错误处理代码)
     *  安全组合模式:
    ...
    public void usageScenarios() {
        System.out.println("组合模式正是应树形结构而生, 所以组合模式的使用场景就是出现树形结构的地方。");
    }
